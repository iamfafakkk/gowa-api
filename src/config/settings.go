...
var (
	...
	// authentication (users, sessions, tokens). It is separate from the chat
	// storage DB so an auth wipe or migration never touches message data.
	...
	// on startup, a single admin user is created with these credentials. Useful
	// for first-boot provisioning without manual DB manipulation.
